...
type EmilyEventContract = {
  [EmilyEvent.SearchReceived]: { correlationId: string; source: string; resultCount: number; timestamp: string }
  [EmilyEvent.SearchTransformed]: { correlationId: string; inputCount: number; outputCount: number; timestamp: string }
  [EmilyEvent.TransformFailed]: { correlationId: string; error: string; timestamp: string }
  [EmilyEvent.SearchReturned]: { correlationId: string; trackCount: number; timestamp: string }
}
...
export class EmilyPublisher {
  ...
  async searchReceived(correlationId: string, source: string, resultCount: number): Promise<void> {
  ...
  async searchTransformed(correlationId: string, inputCount: number, outputCount: number): Promise<void> {
    await this.publisher?.publish(
  ...
  async searchReturned(correlationId: string, trackCount: number): Promise<void> {
    await this.publisher?.publish(
