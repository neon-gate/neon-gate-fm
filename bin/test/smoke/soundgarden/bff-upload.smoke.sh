...
response="$(curl -sS -w "\n%{http_code}" -X POST "${BFF_URL}/api/soundgarden/tracks/upload" \
  -F "file=@$TEST_FILE")"
...
if [[ "$http_code" != "200" ]]; then
  echo "BFF upload failed: expected status 200, got $http_code"
  echo "Response: $body"
  exit 1
fi
...
echo "OK BFF upload status=$http_code trackId=$track_id"
