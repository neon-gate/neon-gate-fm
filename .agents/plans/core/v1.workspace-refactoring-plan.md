...
### 7.0 README Quality Standard

**Every README created or updated in this plan must match the quality of the root [README.md](../../../README.md).** That means:

| Element | Requirement |
|---------|-------------|
| **Tables** | Use markdown tables for structured data (e.g., API reference, event catalog, port allocation, env vars) |
| **Diagrams** | Include mermaid diagrams where topology, flows, or architecture benefit from visualization |
| **Emojis** | Use section emojis for visual hierarchy and quick scanning (e.g., 🗂️ Structure, 🏛️ Architecture, 🐳 Docker) |
| **Formatting** | Clear headings, code blocks with language tags, blockquotes for callouts, horizontal rules for section breaks |
| **Tone** | Consistent, informative, and navigable — same polish as the root README |

Apply this standard to all package READMEs (7.1–7.3) and microservice READMEs (7.4).
...
- **Do not touch `@pack/kernel`** -- use its abstractions, never modify the package
- **Do not touch the Pulse frontend** (`repos/apps/pulse/` app code) -- only update `package.json` dependencies if the refactor requires it
...
## Best Order for Codex to Validate

When Codex picks up this refactor mid-stream, it should validate from the workspace root in the following order so failures are isolated from the package graph outward to app consumers:

1. **Workspace / package manager config**
   - Validate `package.json`, `pnpm-workspace.yaml`, and `turbo.json`
   - Confirm all renamed packages are still included in the workspace
   - Confirm deleted `repos/environment/**` references are gone
2. **Dependency install**
   - Run install validation after workspace/package names are confirmed
   - Fix lockfile or workspace resolution issues before checking TypeScript
3. **TS path aliases**
   - Validate root and per-project `tsconfig*.json` path mappings
   - Confirm imports resolve from `@pack/environment`, `@pack/event-inventory`, and other shared packages
4. **Shared packages build / exports**
   - Build and validate package entrypoints first:
     - `repos/packages/environment/`
     - `repos/packages/event-inventory/`
     - `repos/packages/nats-broker-messaging/`
     - Any other shared package touched by the refactor
   - Confirm `package.json` `main`, `types`, and `exports` remain consistent with emitted files
5. **Apps/web**
   - In this repo, validate the frontend app at `repos/apps/pulse/`
   - Only check dependency resolution and startup/build compatibility required by the workspace refactor
6. **agents/shinoda**
   - Validate `repos/agents/shinoda/` after shared packages and app consumers pass
   - Focus on package resolution, env usage, and event-inventory imports
7. **Docker / devcontainer only if needed for startup parity**
   - Leave `repos/packages/environment-orchestration/docker-compose.yml`, `bin/docker/*.sh`, and any devcontainer parity checks for the end
   - Only use this step when package/build validation passes but runtime startup parity is still in doubt

### Validation Intent

This order is deliberate:

- **Start with workspace truth** so package discovery and install errors fail fast
- **Build shared packages before consumers** so downstream app failures are not hiding export issues
- **Check `repos/apps/pulse/` before `repos/agents/shinoda/`** because the app is a broader workspace consumer and should stay minimally impacted
- **Use Docker/devcontainer validation last** because it is slower and only needed once static/build correctness is already established
